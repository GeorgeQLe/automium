...
- Next automated step: none; Phase 1 is ready to close.
...
- [x] Step 1.5: **Automated** Make the benchmark contract suites pass against the owned-target fixtures and verify that the docs and task files no longer describe third-party SaaS provisioning as a core benchmark dependency.
...
Phase 1 verification completed on 2026-04-07.

- Verified command: `pnpm test:run -- packages/contracts/tests packages/benchmark/tests`.
- Result: benchmark contract suites passed against the owned-target fixtures with 4 passing files / 10 passing tests.
- Wording audit: targeted searches across `docs/benchmarks/`, `tasks/`, and `specs/owned-parity-benchmark-products.md` confirmed that Phase 1 docs and task files no longer frame third-party SaaS provisioning as a core benchmark dependency.
- Milestone status: Phase 1 is ready to close. The next automated work should begin with Phase 2 parity-audit artifacts.
