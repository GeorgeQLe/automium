...
- `compilePlannerIntent(input)` — validates intent, maps to opcode, returns `ExecutorAction`

**Vision domain** (`packages/vision/src/vision-domain.ts`):
- `shouldRequestTargetedVision(input)` — triggers if confidence < 0.7 or actionability failures
- `createTargetedCropRequest(input)` — wraps crop request with schema version, `fullPage: false`

**Frozen contracts** (`packages/contracts/src/`):
- `SemanticSnapshot` — full contract with frameHierarchy, taskContext, checkpointContext, interactiveElements (11 fields each), recentMutations, relevantNetworkEvents, pinnedInvariants
- `ReplayEvent` — step-by-step causal event log with plannerIntent, executorAction, artifact refs, verdict
